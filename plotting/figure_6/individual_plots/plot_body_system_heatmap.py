...
sys.path.insert(0, os.path.dirname(__file__))                           # this dir
...
# Order in which dominant-body-group blocks are stacked top->bottom.
TOP_GROUP_ORDER = ["legs", "torso", "arms", "head"]


def sort_by_top_group(heatmap_df, group_order=TOP_GROUP_ORDER):
    """Group rows by which body group is their row-max, blocks ordered by
    group_order (legs first, then torso, arms, head); within each block sort
    by dominance (gap between top and 2nd group) descending."""
    if heatmap_df.empty:
        return heatmap_df
    dominance = compute_row_dominance_scores(heatmap_df)
    ...
    rows = []
    for idx in heatmap_df.index:
        row = heatmap_df.loc[idx, BODY_GROUPS].astype(float)
        top = None if row.isna().all() else str(row.idxmax())
        rows.append((idx, rank.get(top, len(group_order)), -float(dominance.get(idx, -np.inf))))
    rows.sort(key=lambda r: (r[1], r[2]))
    return heatmap_df.reindex([r[0] for r in rows])
...
def main():
    ...
    parser.add_argument(
        "--csv-path", type=str,
        default=os.path.join(os.path.dirname(__file__), "..", "..", "..", "results", "masking_ablation_pearson.csv"),
    )
    parser.add_argument(
        "--output-dir", type=str,
        default=os.path.join(os.path.dirname(__file__), "figure6a"),
    )
    ...
    both_heatmap = sort_by_top_group(both_heatmap)
